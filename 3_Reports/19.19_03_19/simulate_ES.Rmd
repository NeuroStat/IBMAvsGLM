...
```{r 'Monte-Carlo', eval = FALSE, cache=TRUE}
# Empty data frame
results <- data.frame() %>% 
  as_tibble()
...
```{r 'figure-B', eval=FALSE, echo = FALSE, fig.align='center'}
# Make a plot for B
results %>%
...
```{r 'figure-SigmB', eval=FALSE, echo = FALSE, fig.align='center'}
results %>%
...
In a table, we have:
```{r 'results-table-SigmB', eval=FALSE}
results %>%
  dplyr::select(-B,-TrueB) %>%
  group_by(TrueTau2, estimatorBS) %>%
  summarise(AvgEstTau2 = mean(tau2))
