...
Now we want to construct a confidence interval around the T-map through:
...
```{r echo=FALSE}
library(ggplot2)
hist(S30TVal.lvl3, main='Distribution of T-values at the third level when N = 100 and K = 10', xlab='T-value')
```

The emperical coverages though look very weird: further investigation is needed!

```{r echo=1}
CI.coverages
ggplot(CI.coverages, aes(x=SampleSize, y=Mean,group=Studies)) + 
	geom_line(aes(colour=Studies), size=1.3) + 
