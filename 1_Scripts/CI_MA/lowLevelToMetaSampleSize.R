####################
#### TITLE:     Simulate null data from low subject level to meta-analysis. Calculate coverage: increasing sample size and k studies.
#### Contents:
####
#### Source Files: HPC - Version
#### First Modified: 09/02/2016
#### Notes:
#################
...
# Let us have 1000 simulations per scenario.
# We let the sample size increase from 10 subjects to 100 in steps of 10
# Then the amount of studies in the meta-analysis goes from 2 to 10.
# However, we want a balanced design in the meta-analysis.
# Hence we only take combinations of sample size and studies that return integer values when divided (eg. 20 subjects in 2 studies, not 20 subjects in 3 studies).
...
if(MACHINE=='MAC'){
  wd <- '/Volumes/2_TB_WD_Elements_10B8_Han/PhD/Simulation/Results/Take3'
}
setwd(wd)
...
for(t in 1:nstud){
  ...
  for(s in 1:nsub){
    ...
    WeightSeed <- ((nstud*nstud) + (starting.seed + nsub)) + ((t-1)*nstud) + (s)
      ...
      # Weights 				MIND THE INDEX OF NOISE HERE!!!!!
      weights <- rbind(weights,subjectfMRINoise(Noise[[1]],seed = WeightSeed))
      # Radius
...
save(SCOPE, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_SCOPE_',K,sep=''))
save(SVARCOPE, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_SVARCOPE_',K,sep=''))
save(STMAP, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_STMAP_',K,sep=''))
save(SHEDGE, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_SHEDGE_',K,sep=''))
save(WeightedAvg, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_WeightedAvg_',K,sep=''))
save(varWeightAvg, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_varWeightAvg_',K,sep=''))
save(CI.upper.norm, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_CI.upper.norm_',K,sep=''))
save(CI.lower.norm, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_CI.lower.norm_',K,sep=''))
save(CI.upper.t, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_CI.upper.t_',K,sep=''))
save(CI.lower.t, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_CI.lower.t_',K,sep=''))
save(CI.upper.weightVar, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_CI.upper.weightVar_',K,sep=''))
save(CI.lower.weightVar, file=paste(wd,'/Results/',K,'/SCEN_',SCEN,'/SK_CI.lower.weightVar_',K,sep=''))
