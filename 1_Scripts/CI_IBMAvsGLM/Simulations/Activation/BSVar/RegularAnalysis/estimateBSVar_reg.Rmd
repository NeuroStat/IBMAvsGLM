...
#   no extra term for moderator
# This function is used with purrr to apply over all voxels
fitDL <- function(data){
  fit <- rma(yi = data$hedgeG, vi = data$varG, 
             method = 'DL', data = data)
...
> Note: this report uses a regular analysis without control for some studies coming from the same laboratory. See section on moderator analysis for more information.
...
* 2) Study of Maumet et al., 2016 is about the *nidm* data structure. However, it contains **21** studies about pain. Note that these come from the same laboratory. In this report, we explicitly do not control for this. Need to verify where these studies came from. 
...
# Note, we do not use this variable here!
...
# To do so, we use our custom function fitDL. This takes data from each voxel,
...
# Note, we do not use this variable.
