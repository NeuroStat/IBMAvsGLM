...
To obtain $X$, we use _neuRosim_. In the section/code below, the design matrix is denoted as _pred - base_. Furthermore $\beta_0$ is denoted as _base_. First we set the following parameters:
...
Also note that $\sigma^*$ corresponds to between-subject variability. Hence, this is equal to variability of the estimated first level parameters. We thus get:
\begin{align}
\sigma^{*2} & = \text{Var}(\widehat{\beta_1}) \\ \nonumber
& = \sigma^2(X'X)^{-1}
\end{align}
...
Standardized effect sizes at the second level (study level) are generally calculated by dividing the mean effect with the standard deviation. Or by multiplying the _t_-value with $\frac{1}{\sqrt{n}}$. 
In our case, this corresponds to:

\begin{align}
\text{g} & = \frac{\text{t}}{\sqrt{n}} \\ \nonumber
& = \text{t}(X_G'X_G)^{-1} \\ \nonumber
...
Or simplified:
\begin{align}
\text{g} = \frac{\hat\beta^{*}}{\sigma \sqrt{(X'X)^{-1}}}.
\end{align}

The expected value over all simulations is equal to:
\begin{align}
\frac{\sum_{m = 1}^M\text{g}_m}{M} = \frac{3}{100 \sqrt{(X'X)^{-1}}}.
\end{align}
